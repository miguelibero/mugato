...
namespace mugato {
    ...
    buffer DebugFontAtlasConfigurator::createBuffer()
    {
        return buffer(std::initializer_list<uint8_t>{
    ...
    void DebugFontAtlasConfigurator::setup(Context& ctx)
    {
        setup(ctx.getLabels().getAtlases(),
            ctx.getGorn().getMaterials(),
            ctx.getGorn().getFiles());
    }

    void DebugFontAtlasConfigurator::setup(
